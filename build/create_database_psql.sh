...
psql -c "CREATE ROLE $USER WITH LOGIN PASSWORD '$PASSWORD' CREATEDB CREATEROLE;"
psql -c "CREATE DATABASE $DATABASE OWNER $USER;"

cat schema.sql | psql -U $USER

psql -c "CREATE SEQUENCE IF NOT EXISTS user_roles_id_seq OWNED BY user_roles.id;"
psql -c "ALTER TABLE user_roles ALTER COLUMN id SET DEFAULT nextval('user_roles_id_seq');"

echo
psql -U $USER -c "SELECT pg_size_pretty(pg_table_size(core.oid)) AS size_tables,
